...
<form class="form-horizontal" id="form" method="post" action="<?php echo $action; ?>">
	...
	<div class="row">
					...
					?>
							...
							<div class="col-md-12">
		                        <div class="form-group row" id="role-checkbox">
		                            <label for="colFormLabel" class="col-md-2 col-sm-4 col-form-label">Permissions:</label>
		                            <div class="col-sm-12" id="permission-module">
		...
		<div class="col-md-8" id="service-checkbox">
			<div class="tile">
				<div class="tile-body">
...
<script type="text/javascript">
	...
	var new_roles = "<?php echo $new_roles; ?>";
	if(new_roles != "") {
		var roles = new_roles.split(",");
	} else {
		var roles = new Array();
	}
	...
	{
		var menus = "";
		menus += '<div class="custom-control custom-checkbox checkbox-success form-check">';
		menus += '<input type="checkbox" class="custom-control-input" id="selectall" />';
		menus += '<label class="custom-control-label" for="selectall">Select All</label>';
		menus += '</div>';
		var no = 0;
		$("li.permission").each(function(){
        	...
        	var isChecked = (roles.indexOf(val) !== -1) ? "checked" : "";

			menus += '<div class="custom-control custom-checkbox checkbox-success form-check">';
			menus += '<input type="checkbox" class="custom-control-input" name="roles[]" id="permission'+no+'" value="'+val+'" ' + isChecked + ' />';
			menus += '<label class="custom-control-label" for="permission'+no+'">'+title+'</label>';
			menus += '</div>';
		});
		$("#permission-module").html(menus);
	}
</script>
<?= $this->endSection(); ?>